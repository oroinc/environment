...
behat)
  ...
  set +e
  files=$(grep -e "^application/" -e "^package/" -e "^environment/" -e "^environment/" -e "^Jenkinsfile" -e "^.jenkins" -e "^documentation/code_examples/" "$DIR_DIFF/$FILE_DIFF" | grep -v -E "\.(msi|ods|psd|bat|gif|gitignore|gitkeep|jpg|jpeg|md|mp4|png|py|rst|txt|gliffy)$")
  set -e
  if [[ "${files}" ]]; then
    echo "Changes were detected"
  else
