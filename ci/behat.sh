...
case "${STEP}" in
  check)
    ...
    { set +e; files=$(grep -e "^application/" -e "^package/" -r --exclude=\*.{msi,ods,psd,bat,gif,gitignore,gitkeep,html,jpg,jpeg,md,mp4,png,py,rst,txt,gliffy} "${BUILD_DIR}/ci/artifacts/${PROJECT_NAME}/diff.log"); set -e; }
    if [[ "${files}" ]]; then
